...
splitter_level2 = splitter_level1
splitter_level3 = splitter_level1
...
class Combiner:
    ...
    def combine(self):
        ...
        dfAll = pkl_utils._load(config.INFO_DATA)            # "id", "is_duplicate"
        ...
        if feat_cv_cnt > 0:
            ...
            X_tmp = dfAll_cv_all.drop(["id","is_duplicate"], axis=1).values.astype(float)
        ...
        for run in range(1,self.n_iter+1):
            ...
            if feat_cv_cnt > 0:
                ...
                X_tmp = dfTrain_cv.drop(["id","is_duplicate"], axis=1).values.astype(float)
                if run == 1:
...
class StackingCombiner:
    ...
    def combine(self):
        ...
        self.X_test = dfTest.drop(["id","is_duplicate"], axis=1).values.astype(float)
        if self.meta_feature_dict:
        ...
        for run in range(1,self.n_iter+1):
            ...
            self.y_train_cv[run-1] = dfTrain_cv["is_duplicate"].values.astype(float)
            self.X_train_cv[run-1] = dfTrain_cv.drop(["id","is_duplicate"], axis=1).values.astype(float)
